...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    pts_j_td = pts_j - (td - td_j) * velocity_j; // 进行时间偏移的误差的矫正
    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i; // 根据逆深度，在相机坐标系下投影出3D点
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; // 通过相机外参变换到IMU坐标系
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; // 通过IMU的位姿变换到世界坐标系下
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); // 通过j的逆变换（j的IMU的位姿），变换到j的IMU坐标系下
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); // 通过j的逆变换（j的外参），变换到j的相机坐标系下
    ...
    residual = (pts_camera_j / dep_j).head<2>() - pts_j_td.head<2>(); // 构造2维的视觉重投影残差
#endif

    residual = sqrt_info * residual; // 左乘信息矩阵（权重）（Ceres不支持协方差操作）
