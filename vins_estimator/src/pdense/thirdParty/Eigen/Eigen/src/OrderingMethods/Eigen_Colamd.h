...
/* knobs [0] and stats [0]: pdense row knob and output statistic. */
#define COLAMD_DENSE_ROW 0

/* knobs [1] and stats [1]: pdense column knob and output statistic. */
...
static inline void colamd_set_defaults(double knobs[COLAMD_KNOBS])
{
  ...
  knobs [COLAMD_DENSE_ROW] = 0.5 ;  /* ignore rows over 50% pdense */
  knobs [COLAMD_DENSE_COL] = 0.5 ;  /* ignore columns over 50% pdense */
}
...
static bool colamd(Index n_row, Index n_col, Index Alen, Index *A, Index *p, double knobs[COLAMD_KNOBS], Index stats[COLAMD_STATS])
{
  ...
  Index n_col2 ;    /* number of non-pdense, non-empty columns */
  Index n_row2 ;    /* number of non-pdense, non-empty rows */
  ...
  /* === Initialize scores, kill pdense rows/columns ======================= */
...
/*
  Kills pdense or empty columns and rows, calculates an initial score for
...
static void init_scoring
  (
    ...
    Index *p_n_row2,    /* number of non-pdense, non-empty rows */
    Index *p_n_col2,    /* number of non-pdense, non-empty columns */
    ...
    )
{
  ...
  /* Put the pdense columns at the end, in their natural order */
  for (c = n_col-1 ; c >= 0 ; c--)
  {
    ...
    {
      /* this is a pdense column, kill and order it last */
      Col [c].shared2.order = --n_col2 ;
  ...
  /* of "live" (non-pdense) columns in each row.  No empty rows exist. */
  ...
  for (c = n_col-1 ; c >= 0 ; c--)
  {
    ...
    if (col_length == 0)
    {
      /* a newly-made null column (all rows in this col are "pdense" */
